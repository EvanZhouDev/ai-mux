...
import { muxApiKeysForProvider, muxModels, roundRobinStrategy } from 'ai-mux';
import { createOpenAI } from '@ai-sdk/openai';
import { createAnthropic } from '@ai-sdk/anthropic';

// openai automatically cycles between the two API Keys for *all* OpenAI models
const openai = muxApiKeysForProvider({
  keys: [API_KEY_1, API_KEY_2],
  createProvider: (apiKey) => createOpenAI({ apiKey }),
});

// Grab any model from the muxed provider.
const model = openai('gpt-5.1');

// Alternatively, mux between different providers/models:
const mixedModel = muxModels({
...
const { text } = await generateText({ model, prompt: 'hello there' });
```
...
- `muxApiKeysForProvider` only retries when the error has one of these status codes: 400, 403, 429, 500, 503, 504.
  - See https://ai.google.dev/gemini-api/docs/troubleshooting for more information on these errors.
- `muxModels` can enable the same behavior with `retryOnError: true`.
